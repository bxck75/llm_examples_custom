...
def configure_retriever(uploaded_files):
    ...
    for file in uploaded_files:
        ...
        # Set the path for loading text files
        ...
        # Execute the GenericLoader

        docs.extend(py_loader.load())
    # Split
    python_splitter = RecursiveCharacterTextSplitter.from_language(
        language=Language.PYTHON, chunk_size=chunk_size, chunk_overlap=8
    )
    # Split documents
...
uploaded_files = st.sidebar.file_uploader(
    label="Upload python files", type=["py"], accept_multiple_files=True
)
...
# Set up the StuffDocumentsChain
...
qa_chain2 = ConversationalRetrievalChain(
    memory=memory,
...
if user_query := st.chat_input(placeholder="Ask me anything!"):
    ...
    with st.chat_message("assistant"):
        ...
        response = qa_chain2.run(user_query, callbacks=[retrieval_handler, stream_handler ])
